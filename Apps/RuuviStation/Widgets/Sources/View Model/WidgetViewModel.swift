...
public extension WidgetViewModel {
    internal func widgetDeepLinkURL(
        sensorId: String?,
        record: RuuviTagSensorRecord?
    ) -> URL? {
        let resolvedSensorId = record?.macId?.value ?? record?.luid?.value ?? sensorId
        guard let resolvedSensorId, !resolvedSensorId.isEmpty else {
            return nil
        }

        var components = URLComponents()
        components.scheme = "ruuviwidget"
        components.host = "open"
        components.queryItems = [
            URLQueryItem(name: "sensorId", value: resolvedSensorId),
        ]
    ...
    func getSensor(from config: SingleSensorWidgetConfiguration) -> WidgetSensorEnum? {
        if let identifier = config.sensorSelectionIdentifier,
    ...
    func measurementShortName(from config: SingleSensorWidgetConfiguration) -> String {
        let appSettings = getAppSettings()
        guard let variant = selectedVariant(
            from: config,
            appSettings: appSettings
        ) else {
            return shortName(for: defaultVariant(appSettings: appSettings))
        }
        return shortName(for: variant)
    }

    func getUnit(from config: SingleSensorWidgetConfiguration) -> String {
        let appSettings = getAppSettings()
        guard let variant = selectedVariant(
            from: config,
            appSettings: appSettings
    ...
    internal func largeWidgetIndicators(from entry: WidgetEntry) -> [WidgetIndicatorDisplayItem] {
        ...
        let selectedCodes: [RuuviCloudSensorVisibilityCode] = {
            guard let identifier = entry.config.sensorSelectionIdentifier,
    ...
    internal func measurementTime(from entry: WidgetEntry) -> String {
        measurementTime(from: entry.record?.date)
    }
...
extension WidgetViewModel {
    ...
    private func selectedVariant(
        from config: SingleSensorWidgetConfiguration,
    ...
    private func resolvedDeviceType(for entry: WidgetEntry) -> RuuviDeviceType {
        if entry.config.deviceType == .ruuviAir {
            return .ruuviAir
        }
    ...
    private func resolvedVisibleVariants(
        ...
        let displayOrderCodes = localSettings?.displayOrder ?? cloudSettings?.displayOrderCodes
        let defaultDisplayOrder = localSettings?.defaultDisplayOrder ?? cloudSettings?.defaultDisplayOrder

        let preferenceVariants = uniqueOrderedVariants(
            (displayOrderCodes ?? []).compactMap { code in
        ...
        if defaultDisplayOrder == false, !preferenceVariants.isEmpty {
