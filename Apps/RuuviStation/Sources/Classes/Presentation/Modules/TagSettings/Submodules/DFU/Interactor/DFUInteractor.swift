...
extension DFUInteractor: DFUInteractorInput {
    ...
    func waitForAirDevice(
        ...
        guard deviceType == .ruuviAir else {
            log("Wait for Air device skipped: device type is not RuuviAir.")
        ...
        return Future { [weak self] promise in
            ...
            func complete(_ result: Result<Void, Error>) {
                ...
                switch result {
                case .success:
                    self.log("Air device detected. Stopping scan.")
                case let .failure(error):
                    self.log("Air device wait finishing with error: \(error.localizedDescription)")
                }
                self.stopAirScan()
                switch result {
                case .success:
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 5) { [weak self] in
                ...
                guard let foreground else {
                    self.log("Foreground not available. Resolving Air wait immediately.")
                    complete(.success(()))
                    return
                }

                self.log("Starting Air device scan after delay: \(self.deviceDescription(for: ruuviTag))")
                self.stopAirScan()
                ...
                self.airScanTimeoutTimer = Timer.scheduledTimer(
                    ...
                    guard let self else { return }
                    self.log("Timeout reached while waiting for Air device (\(timeout)s).")
...
extension DFUInteractor {
    private func handleAirAdvertisement(
        ...
        log("Air scan advertisement received: \(deviceDescription(for: advertisement))")

        if isSameDevice(advertisement, target) {
            log("Air scan target matched.")
            onMatch()
        }
    }
    ...
    private func stopAirScan() {
        let invalidate: (DFUInteractor) -> Void = { interactor in
            let hadActivity = interactor.airScanToken != nil || interactor.airScanTimeoutTimer != nil
            ...
            if hadActivity {
                interactor.log("Air scan stopped.")
            }
        }
    ...
    private func deviceDescription(for tag: RuuviTagSensor) -> String {
        let mac = tag.macId?.value ?? "unknown-mac"
        let luid = tag.luid?.value ?? "unknown-luid"
        return "mac: \(mac), luid: \(luid)"
    }

    private func deviceDescription(for tag: RuuviTagSensorRecord) -> String {
        let mac = tag.macId?.value ?? "unknown-mac"
        let luid = tag.luid?.value ?? "unknown-luid"
        return "mac: \(mac), luid: \(luid)"
    }
