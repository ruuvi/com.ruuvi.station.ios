...
final class CardsSettingsState: ObservableObject {
    ...
    init(
        ...
        self.name = snapshot.displayData.name
        self.ownerName = snapshot.ownership.ownerName.unwrapped
        self.showOwner = snapshot.ownership.isAuthorized
        self.showOwnersPlan = CardsSettingsState.shouldShowOwnersPlan(
            ownership: snapshot.ownership,
            metadata: snapshot.metadata
        )
        self.ownersPlan = snapshot.ownership.ownersPlan.unwrapped
        self.showShare = snapshot.metadata.canShareTag
        self.shareSummary = Self.calculateShareSummary(from: snapshot)
        self.backgroundImage = snapshot.displayData.background.map { Image(uiImage: $0) }
        self.moreInfoRows = Self.buildMoreInfoRows(from: snapshot)
        self.firmwareVersion = snapshot.displayData.firmwareVersion.unwrapped
        ...
        self.showKeepConnectionStatusLabel = !snapshot.capabilities.hideSwitchStatusLabel

        // Initialize offset values
        self.temperatureOffset = snapshot.calibration.temperatureOffset.unwrapped
        self.humidityOffset = snapshot.calibration.humidityOffset.unwrapped
        self.pressureOffset = snapshot.calibration.pressureOffset.unwrapped
        self.showHumidityOffset = snapshot.calibration.isHumidityOffsetVisible
        self.showPressureOffset = snapshot.calibration.isPressureOffsetVisible
        self.hasLatestMeasurement = snapshot.latestRawRecord != nil
        bind(to: snapshot)
    }
