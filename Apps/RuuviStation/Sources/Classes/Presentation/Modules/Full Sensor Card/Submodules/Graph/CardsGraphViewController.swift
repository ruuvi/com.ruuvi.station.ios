...
extension CardsGraphViewController: CardsGraphViewInput {
    ...
    func setChartViewData(
        ...
        for data in chartViewData {
            ...
            let displayType = displayMeasurementType(for: data.variant, settings: settings)
            populateChartView(
                from: data,
                displayType: displayType,
                unit: unit(for: data.variant, settings: settings),
                settings: settings,
                view: view
            )
        }
    ...
    func updateLatestMeasurement(
        ...
        entries.forEach { variant, entry in
            ...
            view.updateLatest(
                with: entry,
                type: displayMeasurementType(for: variant, settings: settings),
...
extension CardsGraphViewController {
    ...
    private func displayMeasurementType(
        for variant: MeasurementDisplayVariant,
        settings: RuuviLocalSettings
    ) -> MeasurementType {
        variant.type
    ...
    private func calculateMinMaxForChart(for view: CardsGraphView) {
        ...
        let measurementType = variant(for: view)
            .map { displayMeasurementType(for: $0, settings: settings) } ?? view.measurementType
