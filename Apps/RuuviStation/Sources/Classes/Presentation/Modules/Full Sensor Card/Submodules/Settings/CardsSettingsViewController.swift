...
private extension CardsSettingsViewController {
    func handleAlertRangeChange(_ change: AlertRangeChange) {
        guard change.lowerBound < change.upperBound else { return }
        if let current = sliderConfiguration(for: change.alertType)?.selectedRange {
            let lowerDiff = abs(current.lowerBound - change.lowerBound)
            let upperDiff = abs(current.upperBound - change.upperBound)
            if lowerDiff < 0.0001, upperDiff < 0.0001 {
                return
            }
        }
    ...
    func alertSection(for alertType: AlertType) -> CardsSettingsState.AlertSectionModel? {
        state.alertSections.first { $0.alertType.rawValue == alertType.rawValue }
    }

    func sliderConfiguration(for alertType: AlertType) -> AlertSliderConfiguration? {
