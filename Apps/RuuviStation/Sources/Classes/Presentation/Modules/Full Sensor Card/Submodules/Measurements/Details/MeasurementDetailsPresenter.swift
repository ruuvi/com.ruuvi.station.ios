...
extension MeasurementDetailsPresenter: MeasurementDetailsPresenterInput {
    ...
    func configure(
        ...
        self.measurementVariant = variant
        self.measurementVariant = variant ?? defaultVariant(for: measurementType)
...
private extension MeasurementDetailsPresenter {
    ...
    func updateChart() {
        ...
        let displayType = displayMeasurementType(for: variant)
        let unitSymbol = unitSymbol(for: variant)
        view?.setChartData(chartData, settings: settings, displayType: displayType, unit: unitSymbol, measurementService: measurementService)
    }
...
private extension MeasurementDetailsPresenter {
    ...
    func displayMeasurementType(for variant: MeasurementDisplayVariant) -> MeasurementType {
        variant.type
    }

    // swiftlint:disable:next cyclomatic_complexity
    func unitSymbol(for variant: MeasurementDisplayVariant) -> String {
        switch variant.type {
        case .temperature:
