...
class MeasurementDetailsPresenter: NSObject {
    ...
    private let dataQueue = DispatchQueue(label: "com.ruuvi.measurementdetails.data", attributes: .concurrent)
    ...
    // Operations tracking
    private var currentLoadOperation: Operation?
    private var currentSyncOperation: Operation?
...
extension MeasurementDetailsPresenter: MeasurementDetailsPresenterInput {
    ...
    func start() {
        guard !isViewActive else { return } // Prevent duplicate starts
...
//        currentLoadOperation = checkOp
        ...
        checkOp.on(success: { [weak self] results in
            ...
            print("Error checking data count: \(error)")
            self?.handleDataLoadError()
            completion?()
        })
    }
...
//        currentLoadOperation = op
        ...
        op.on(success: { [weak self] results in
            ...
            print("Error loading downsampled data: \(error)")
            self?.handleDataLoadError()
        }, completion: {
            completion?()
        })
...
private extension MeasurementDetailsPresenter {
    ...
    func startUpdateTimer() {
        DispatchQueue.main.async { [weak self] in
            ...
            let interval = self.settings.appIsOnForeground ? 2 : self.settings.chartIntervalSeconds
            self.updateTimer = Timer.scheduledTimer(
                withTimeInterval: TimeInterval(interval),
                repeats: true
            ) { [weak self] _ in
                self?.loadLatestMeasurements()
            }
        }
    }
...
//        appStateToken = NotificationCenter.default.addObserver(
...
//        }
    ...
    func removeAllObservers() {
        ...
        if let appStateToken = appStateToken {
            NotificationCenter.default.removeObserver(appStateToken)
            self.appStateToken = nil
        }
    }
}
