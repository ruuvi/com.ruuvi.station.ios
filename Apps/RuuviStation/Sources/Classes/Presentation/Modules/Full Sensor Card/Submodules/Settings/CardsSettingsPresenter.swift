...
class CardsSettingsPresenter: NSObject, CardsSettingsPresenterInput {
    ...
    func configure(
        ...
        view?.configure(
            snapshot: snapshot,
            dashboardSortingType: self.settings.dashboardSensorOrder.count == 0 ? .alphabetical : .manual
        )
...
extension CardsSettingsPresenter: CardsSettingsViewOutput {
    ...
    func viewDidAskToRemoveRuuviTag() {
        guard let sensor else { return }
        router?.openSensorRemoval(ruuviTag: sensor, output: self)
    }
    ...
    func viewDidTapOnTxPower() {
        guard let snapshot else { return }
        if snapshot.displayData.txPower != nil {
            // No additional action; tapping provides feedback only when value exists
            return
        } else {
            viewDidTriggerFirmwareUpdateDialog()
        }
    ...
    func viewDidTapOnMeasurementSequenceNumber() {
        guard let snapshot else { return }
        if snapshot.displayData.measurementSequenceNumber != nil {
            // Nothing else to do for existing value
            return
        } else {
            viewDidTriggerFirmwareUpdateDialog()
        }
    ...
    func viewDidTapOnOwner() {
        guard let sensor, let snapshot else { return }
