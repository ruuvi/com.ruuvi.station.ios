...
extension CardsGraphPresenter: CardsGraphViewInteractorOutput {
    ...
    private func updateLatestMeasurement(_ measurement: RuuviMeasurement) {
        let latestEntries = chartModules.reduce(into: [MeasurementDisplayVariant: ChartDataEntry?]()) { result, variant in
    ...
    private func chartEntry(for data: RuuviMeasurement, variant: MeasurementDisplayVariant) -> ChartDataEntry? {
        ...
        case .temperature:
            ...
            if let temp {
                let targetUnit = variant.resolvedTemperatureUnit(default: settings.temperatureUnit.unitTemperature)
    ...
    private func graphMeasurementType(for variant: MeasurementDisplayVariant) -> MeasurementType {
