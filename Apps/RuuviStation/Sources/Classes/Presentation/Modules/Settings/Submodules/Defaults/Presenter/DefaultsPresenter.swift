...
extension DefaultsPresenter {
    ...
    private func buildUseImprovedDashboard() -> DefaultsViewModel {
        let viewModel = DefaultsViewModel()
        viewModel.title = "Use Improved Dashboard"
        viewModel.boolean.value = flags.useImprovedDashboard
        viewModel.hideStatusLabel.value = !settings.showSwitchStatusLabel
        viewModel.type.value = .switcher

        bind(viewModel.boolean, fire: false) { observer, bool in
            observer.flags.useImprovedDashboard = GlobalHelpers.getBool(from: bool)
        }
