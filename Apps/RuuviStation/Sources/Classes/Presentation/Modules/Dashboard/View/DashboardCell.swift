...
struct DashboardCellLayoutConstants {
    ...
    static let prominentViewHeightPhone: CGFloat = 30
    ...
    static func gridHeight(
        ...
        let rowHeight = gridRowHeight(showIndicatorTitle: dashboardType == .image)
        let rowSpacing = gridRowSpacing(showIndicatorTitle: dashboardType == .image)
                ...
                indicator
                    .constrainHeight(
                        constant: DashboardCellLayoutConstants
                            .gridRowHeight(showIndicatorTitle: dashboardType == .image)
                    )
                rowsStackView
                    .addArrangedSubview(
                        indicator
...
extension DashboardCell {
    ...
    private static func calculateImageHeight(
        ...
        // Top padding
        totalHeight += DashboardCellLayoutConstants.topPadding                    // 12

        // Name label
        totalHeight += nameHeight                                                  // variable

        // Spacing after name
        totalHeight += DashboardCellLayoutConstants.groupSpacing                   // 16

        // Prominent view
        totalHeight += DashboardCellLayoutConstants.prominentViewHeight           // 30 or 36

        // Content after prominent view (conditional)
        if hasAQI {
            // Progress view section
            totalHeight += DashboardCellLayoutConstants.progressViewTopSpacing    // 4
            totalHeight += DashboardCellLayoutConstants.progressViewHeight        // 16
            totalHeight += DashboardCellLayoutConstants.progressViewToGridSpacing // 10
        } else {
        ...
        // Grid
        totalHeight += gridHeight                                                 // variable

        // Spacing to footer
        totalHeight += DashboardCellLayoutConstants.gridToFooterSpacing          // 6

        // Footer
        totalHeight += DashboardCellLayoutConstants.footerHeight                  // 24

        // Bottom padding
        totalHeight += DashboardCellLayoutConstants.bottomPadding                 // 4
