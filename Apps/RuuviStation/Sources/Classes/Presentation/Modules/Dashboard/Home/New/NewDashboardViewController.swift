...
final class NewDashboardViewController: UIViewController {
...
private extension NewDashboardViewController {
    ...
    func configureRestartAnimationsOnAppDidBecomeActive() {
        ...
        // appDidBecomeActiveToken = NotificationCenter.default.addObserver(...)
    }
}
...
private extension NewDashboardViewController {
    func reloadCollectionView(redrawLayout: Bool = false) {
        DispatchQueue.main.async { [weak self] in
            guard let self = self else { return }
            if redrawLayout {
                forceCompleteReload()
            } else {
                updateData(with: snapshots, animated: redrawLayout)
            }
        }
    }

    func forceCompleteReload() {
        guard !isDragSessionInProgress else { return }
...
extension NewDashboardViewController: UICollectionViewDelegate {
    ...
    func collectionView(
        ...
        // Currently unused - implement if needed for cell animations
    }
...
extension NewDashboardViewController: NewDashboardViewInput {
    func localize() {
        // No-op for now
    }
...
extension NewDashboardViewController: RuuviTagDashboardCellDelegate {
...
extension NewDashboardViewController: NoSensorViewDelegate {
...
extension NewDashboardViewController: UITextFieldDelegate {
    func textField(
...
extension NewDashboardViewController: DashboardSignInBannerViewDelegate {
