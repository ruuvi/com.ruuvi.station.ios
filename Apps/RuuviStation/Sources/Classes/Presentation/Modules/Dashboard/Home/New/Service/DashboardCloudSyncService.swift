...
extension DashboardCloudSyncService {
    ...
    func enableCloudMode() {
        settings.cloudModeEnabled = true
        delegate?.cloudSyncService(self, cloudModeDidChange: true)
    }

    func disableCloudMode() {
        settings.cloudModeEnabled = false
        delegate?.cloudSyncService(self, cloudModeDidChange: false)
    }
...
extension DashboardCloudSyncService {
    ...
    func getCurrentSyncStatus() -> NetworkSyncStatus {
        // This would need to be implemented based on your actual sync status tracking
    ...
    func setSyncing(_ syncing: Bool) {
        settings.isSyncing = syncing
    }
}

// MARK: - History Sync Management
extension DashboardCloudSyncService {

    func shouldSyncHistoryOnDashboard() -> Bool {
        return settings.historySyncOnDashboard
    }

    func setSyncHistoryOnDashboard(_ sync: Bool) {
        settings.historySyncOnDashboard = sync
    }

    func isHistorySyncLegacy() -> Bool {
        return settings.historySyncLegacy
    }

    func isHistorySyncForEachSensor() -> Bool {
        return settings.historySyncForEachSensor
    }
}
...
extension DashboardCloudSyncService {
    ...
    func registerForPushNotifications() {
        // This would typically be called when starting the service
        // Implementation depends on your push notification setup
    }

    func unregisterPushNotifications() {
        guard let token = pnManager.fcmToken, !token.isEmpty else { return }
...
extension DashboardCloudSyncService {
    ...
    func hasSignedInAtLeastOnce() -> Bool {
        return settings.signedInAtleastOnce
    }

    func setSignedInAtLeastOnce() {
        settings.signedInAtleastOnce = true
    }

    func getUserInfo() -> (isAuthorized: Bool, email: String?) {
        return (ruuviUser.isAuthorized, ruuviUser.email)
    }
}
...
extension DashboardCloudSyncService {
    ...
    func handleSyncError(_ error: Error) {
        if let ruuviError = error as? RuuviServiceError {
            switch ruuviError {
            case .ruuviCloud(.notAuthorized):
                forceLogout()
            default:
                break
            }
    ...
    func handleAuthError(_ error: Error) {
        delegate?.cloudSyncService(self, authorizationFailed: true)
    }
}
