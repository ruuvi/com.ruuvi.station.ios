...
//
//// swiftlint:disable file_length
//class NewDashboardPresenter: DashboardModuleInput {
...
//        didSet {
//            syncSnapshotsFromViewModels()
//            restartObservingRuuviTagLastRecords()
//        }
//    }
//
...
//        if let viewModel = viewModels.first(where: { $0.id == snapshot.id }) {
//            return viewModel
//        }
//        return nil
//    }
...
//    // MARK: - Sync Methods
//
//    /// Sync snapshots from viewModels during transition period
//    private func syncSnapshotsFromViewModels() {
//        let newSnapshots: [RuuviTagCardSnapshot] = viewModels.compactMap { viewModel in
//            guard let snapshot = RuuviTagCardSnapshotDataBuilder.createSnapshot(
//                from: viewModel,
...
//            ) else { return nil }
//
//            // Ensure alerts are synced for the snapshot
//            if let ruuviTag = ruuviTags.first(where: { $0.id == viewModel.id }) {
//                snapshot.syncAllAlerts(from: alertService, physicalSensor: ruuviTag)
//            }
//
//            return snapshot
//        }
...
//    }
//
//    /// Update specific snapshot from record
//    private func updateSnapshot(from record: RuuviTagSensorRecord, for sensor: RuuviTagSensor) {
//        guard let snapshotIndex = snapshots.firstIndex(where: { $0.id == sensor.id }) else { return }
//
//        let sensorSettings = sensorSettingsList.first(where: {
...
//
//        // Also sync alerts
//        snapshots[snapshotIndex].syncAllAlerts(from: alertService, physicalSensor: sensor)
//        view?.updateSnapshot(from: snapshots[snapshotIndex])
//    }
//
//    // Fix: Load backgrounds after initial build
//    private func loadBackgroundsForViewModels() {
//        for viewModel in viewModels {
//            guard let ruuviTag = ruuviTags.first(where: { $0.id == viewModel.id }) else { continue }
//
//            ruuviSensorPropertiesService.getImage(for: ruuviTag)
//                .on(success: { [weak self] image in
//                    viewModel.background = image
//                    // Update corresponding snapshot background
//                    guard let self = self else { return }
//                    if let snapshotIndex = self.snapshots.firstIndex(where: { $0.id == ruuviTag.id }) {
//                        self.snapshots[snapshotIndex].updateBackgroundImage(image)
//                        self.view?.updateSnapshot(from: self.snapshots[snapshotIndex])
//                    }
...
//
//extension NewDashboardPresenter: NewDashboardViewOutput {
//    func viewDidLoad() {
...
//        guard let viewModel = viewModel(for: snapshot) else { return }
...
//        orderedIds: [String]
//    ) {
//        // Fix: Update settings first, then reorder current data
//        settings.dashboardSensorOrder = orderedIds
//        ruuviAppSettingsService.set(dashboardSensorOrder: orderedIds)
//
//        // Reorder current viewModels and sync snapshots
//        viewModels = reorder(viewModels)
//        view?.dashboardSortingType = dashboardSortingType()
//    }
//
...
//
//extension NewDashboardPresenter: MenuModuleOutput {
...
//
//extension NewDashboardPresenter: SignInBenefitsModuleOutput {
//    func signIn(
...
//}
//
//extension NewDashboardPresenter: DiscoverRouterDelegate {
...
//
//extension NewDashboardPresenter: DashboardRouterDelegate {
...
//
//extension NewDashboardPresenter: RuuviNotifierObserver {
...
//
//extension NewDashboardPresenter: CardsModuleOutput {
...
//
//extension NewDashboardPresenter: TagSettingsModuleOutput {
...
//
//extension NewDashboardPresenter: SensorRemovalModuleOutput {
...
//// MARK: - Private
//
//extension NewDashboardPresenter {
//
...
////                        updatedViewModel.isConnected = self.background.isConnected(uuid: luid.value)
////                    } else if let macId = tag.macId {
////                        updatedViewModel.networkSyncStatus = localSyncState.getSyncStatusLatestRecord(for: macId)
////                        updatedViewModel.isConnected = false
////                    }
//                    ruuviViewModels.append(viewModel)
//                }
//            }
//
//            DispatchQueue.main.async { [weak self] in
//                guard let self = self else { return }
//                let vms = self.reorder(ruuviViewModels)
//                if self.didLoadInitialRuuviTags {
//                    self.view?.showNoSensorsAddedMessage(show: vms.isEmpty)
//                    self.askAppStoreReview(with: vms.count)
//                }
//
//                self.viewModels = vms
//
//                // Sync alerts for all snapshots
//                for (index, snapshot) in self.snapshots.enumerated() {
//                    if let ruuviTag = self.ruuviTags.first(where: { $0.id == snapshot.id }) {
//                        self.snapshots[index].syncAllAlerts(from: self.alertService, physicalSensor: ruuviTag)
//                    }
//                }
//
//                // Fix: Load backgrounds after initial data is shown
//                self.loadBackgroundsForViewModels()
//
//                // Show sign in banner...
//                if let currentAppVersion = self.currentAppVersion() {
//                    self.view?.shouldShowSignInBanner =
//                    self.settings.signedInAtleastOnce && !self.ruuviUser.isAuthorized &&
//                    self.viewModels.count > 0 &&
//                    !self.settings.dashboardSignInBannerHidden(for: currentAppVersion)
//                } else {
//                    self.view?.shouldShowSignInBanner = false
//                }
//            }
//        }
//    }
//
//    private func syncViewModelsBlocking() {
//        view?.dashboardType = settings.dashboardType
//        view?.dashboardTapActionType = settings.dashboardTapActionType
//        view?.dashboardSortingType = dashboardSortingType()
//
...
//        var ruuviViewModels: [CardsViewModel] = []
//        let lock = NSLock() // Thread safety for array access
//
...
//            }
//
//            viewModel.rhAlertLowerBound = alertService.lowerRelativeHumidity(for: ruuviTag)
//            viewModel.rhAlertUpperBound = alertService.upperRelativeHumidity(for: ruuviTag)
//
//            // Inject previous record if available
//            if let previousRecord = viewModels.first(where: { $0.id == ruuviTag.id })?.latestMeasurement {
//                viewModel.update(previousRecord)
//            }
//
//            syncAlerts(ruuviTag: ruuviTag, viewModel: viewModel)
//
//            // 🚨 BLOCKING: Make storage call synchronous
//            let semaphore = DispatchSemaphore(value: 0)
//
//            let op = ruuviStorage.readLatest(ruuviTag)
//            op.on { [weak self] record in
//                defer {
//                    semaphore.signal() // Always signal completion
//                    dispatchGroup.leave()
//                }
//
...
//
//        if let ruuviTagSensor = ruuviTags.first(where: { $0.id == viewModel.id }) {
//            updateSnapshot(from: updatedRecord, for: ruuviTagSensor)
//        }
//    }
//
...
//        print("how many items", viewModels.count)
//        for viewModel in viewModels {
//            if viewModel.type == .ruuvi,
//               let ruuviTagSensor = ruuviTags.first(where: { $0.id == viewModel.id }) {
...
//                    let viewModel = sSelf.viewModels
