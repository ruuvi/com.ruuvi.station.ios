...
//class CardsGraphPresenter: NSObject, CardsGraphModuleInput {
...
//    private var output: CardsGraphModuleOutput?
...
//    func configure(output: CardsGraphModuleOutput) {
//        self.output = output
//    }

    func configure(snapshot: RuuviTagCardSnapshot, sensor: AnyRuuviTagSensor?) {
        self.ruuviTag = sensor
        self.snapshot = snapshot
    }

    func scrollTo(snapshot: RuuviTagCardSnapshot, sensor: AnyRuuviTagSensor?) {
        if interactor.isSyncingRecords() {
            view?.showSyncAbortAlertForSwipe()
            return
        }

//        output?.cardsGraphSafeToSwipe(to: snapshot, module: self)
        configure(snapshot: snapshot, sensor: sensor)
        restartObserving()
    }
...
extension CardsGraphPresenter: CardsGraphViewOutput {
    ...
    func viewDidTriggerSync(for snapshot: RuuviTagCardSnapshot?) {
        ...
        guard let luid = ruuviTag.luid, let snapshot = snapshot else { return }
...
//        view?.updateLatestRecordStatus(with: record)
    ...
    private func chartEntry(for data: RuuviMeasurement, type: MeasurementType) -> ChartDataEntry? {
        ...
        case .aqi:
            value = measurementService.aqi(for: data.co2, pm25: data.pm25)
        case .co2:
