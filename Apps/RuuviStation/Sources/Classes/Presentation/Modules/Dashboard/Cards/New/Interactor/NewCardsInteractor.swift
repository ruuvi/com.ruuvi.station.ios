...
extension NewCardsInteractor {
    ...
    private func fetchLast() {
        ...
        op.on(success: { [weak self] record in
            ...
            sSelf.presenter.createChartModules(from: chartsCases, for: sSelf.ruuviTagSensor)
            sSelf.presenter.updateLatestRecord(record, for: sSelf.ruuviTagSensor)
        }, failure: { [weak self] error in
            guard let sSelf = self else { return }
            self?.presenter.interactorDidError(.ruuviStorage(error), for: sSelf.ruuviTagSensor)
        })
    }
    ...
    private func fetchLastFromDate() {
        ...
        op.on(success: { [weak self] results in
            ...
            guard results.count > 0,
                ...
                self?.presenter.updateLatestRecord(lastMeasurementRecord, for: sSelf.ruuviTagSensor)
                return
            }
            ...
            sSelf.presenter.updateLatestRecord(last, for: sSelf.ruuviTagSensor)
        }, failure: { [weak self] error in
            guard let sSelf = self else { return }
            self?.presenter.updateLatestRecord(lastMeasurementRecord, for: sSelf.ruuviTagSensor)
            self?.presenter.interactorDidError(.ruuviStorage(error), for: sSelf.ruuviTagSensor)
    ...
    private func insertMeasurements(_ newValues: [RuuviMeasurement]) {
        presenter.insertMeasurements(newValues, for: ruuviTagSensor)
    }
