...
struct VoltageMeasurementExtractor: MeasurementExtractor {
    func extract(
        ...
        guard let voltage = record.voltage else { return nil }

        let value = String.localizedStringWithFormat(
            "%.3f",
            measurementService?.double(for: voltage) ?? voltage.converted(to: .volts).value
        )
...
struct AccelerationAxisMeasurementExtractor: MeasurementExtractor {
    ...
    func extract(
        ...
        let value = String.localizedStringWithFormat(
            "%.3f",
            measurement.converted(to: .gravity).value
        )
...
struct IndicatorDataManager {
    ...
    static func createGridConfiguration(
        ...
        let adjustedIndicators: [RuuviTagCardSnapshotIndicatorData] = sortedIndicators.enumerated().map { index, indicator in
            RuuviTagCardSnapshotIndicatorData(
