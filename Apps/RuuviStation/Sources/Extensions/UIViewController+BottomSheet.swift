...
struct SheetConfiguration {
    ...
    /// Creates a configuration with calculated maximum height
    static func withCalculatedMaxHeight(
        additionalTopMargin: CGFloat = 40
    ) -> SheetConfiguration {
        ...
        let navBarHeight: CGFloat = {
            if let keyWindow = window,
               let rootVC = keyWindow.rootViewController {
                ...
                } else if let navController = rootVC.presentedViewController as? UINavigationController {
                    return navController.navigationBar.frame.height
                }
            }
            return 44 // Default navigation bar height
        }()

        let maxHeight = windowHeight - (safeTop + navBarHeight + additionalTopMargin)
