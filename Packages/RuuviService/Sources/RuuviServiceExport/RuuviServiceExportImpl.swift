...
public final class RuuviServiceExportImpl: RuuviServiceExport {
    ...
    public func csvLog(
        ...
        let records: [RuuviTagSensorRecord]
        do {
            ruuviTag = try await ruuviStorage.readOne(uuid)
            records = try await ruuviStorage.readAll(uuid, after: networkPuningDate)
...
extension RuuviServiceExportImpl {
    ...
    private func exportToXlsx(
        ...
        return try await withCheckedThrowingContinuation { continuation in
            queue.async {
                autoreleasepool {
                    let firmwareType = RuuviDataFormat.dataFormat(from: version)
