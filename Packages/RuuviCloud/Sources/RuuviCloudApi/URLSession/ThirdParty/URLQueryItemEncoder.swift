...
extension URLQueryItemEncoder {
    ...
    private func push(_ value: DateComponents, forKey codingPath: [CodingKey]) throws {
        guard (value.calendar?.identifier ?? Calendar.current.identifier) == .gregorian,
            ...
            throw DecodingError.dataCorrupted(
                DecodingError.Context(codingPath: codingPath, debugDescription: "Invalid date components")
            )
        }
    ...
    private func push(_ value: some Encodable, forKey codingPath: [CodingKey]) throws {
...
private extension URLQueryItemEncoder {
    struct KeyedContainer<Key: CodingKey>: KeyedEncodingContainerProtocol {
        ...
        func nestedUnkeyedContainer(forKey key: Key) -> UnkeyedEncodingContainer {
            UnkeyedContanier(encoder: encoder, codingPath: codingPath + [key])
        }

        func superEncoder() -> Encoder {
            URLQueryItemReferencingEncoder(encoder: encoder, codingPath: codingPath)
        }

        func superEncoder(forKey key: Key) -> Encoder {
            URLQueryItemReferencingEncoder(encoder: encoder, codingPath: codingPath + [key])
        }
    }
