...
//swiftlint:disable:next cyclomatic_complexity
    private func chartEntry(for data: RuuviMeasurement) -> ChartDataEntry {
        ...
        return ChartDataEntry(x: data.date.timeIntervalSince1970, y: Double(round(y)))
    }
    ...
    private func setDownSampled(dataSet: [RuuviMeasurement], completion: (() -> Void)? = nil) {
        ...
        for i in 0..<data_length/every {
            ...
            chartData.addEntry(ChartDataEntry(x: max_area_point.0, y: Double(round(max_area_point.1))), dataSetIndex: 0)
