...
class ChartPresentationController: UIPresentationController {
    ...
    private lazy var tapGestureRecognizer: UITapGestureRecognizer = {
        let tap = UITapGestureRecognizer(target: self, action: #selector(ChartPresentationController.dimmingViewTapped(_:)))
        return tap
    }()
    ...
    @objc func dimmingViewTapped(_ tap: UITapGestureRecognizer) {
        if let chart = self.presentedViewController as? ChartViewController {
            chart.output.viewDidTapOnDimmingView()
        }
    }
